...
def make_toc(wdir, relpath, _toc):
	...
	for i, chapter in enumerate(_toc):
		...
		rtn += "    <li class=\"sep\"><a class='navdropdown' href='{2}'>{0} {1}</a>\n".format(i, chapter, path)
		rtn += "    <ul>\n"
		rtn += "      <li class=\"navhome sep\">\n"
...
try:
	...
	with open(os.path.join(includedir,"learn_toc.php"), "w") as ofile:
		...
		ofile.write("<li>"
					+"<a class='navdropdown' href='/learn'>Learn</a>\n")
				...
				with fileinput.FileInput(os.path.join(wd,path,f)) as ifile:
					for line in ifile:
						if line.strip() == "%%%%TOC_REPLACE%%%%":
							if cursection in [None, '']:
								if curchapter in [None, '']:
									mtc = make_toc(cwd, dir_prefix, _toc)
									content += mtc
								else:
